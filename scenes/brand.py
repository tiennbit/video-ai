...
# ===================== COVER / THUMBNAIL SEO (khung "nướng" đầu video) =====================
# TikTok API KHÔNG cho upload ảnh cover rời — cover BẮT BUỘC là 1 khung hình trong video
# (chọn qua video_cover_timestamp_ms). Nên ta render sẵn 1 khung cover chuẩn SEO, giữ TĨNH
# ở ĐẦU video; bên đăng (tiktok_publish/config.COVER_TIMESTAMP_MS) trỏ đúng vào khung này.
COVER_ON = True        # bật khung cover SEO ở đầu mỗi video (đặt False để tắt)
COVER_HOLD = 0.6       # giây giữ khung cover TĨNH — mọi khung trong khoảng này giống hệt nhau
COVER_MAXW = 3.7       # bề ngang tối đa cho chữ hook (safe zone dọc 9:16)
COVER_MAXH = 3.2       # chiều cao tối đa cho khối hook
...
def wrap_lines(text, max_chars=14):
    """Bọc chuỗi thành nhiều dòng ~max_chars ký tự — cho chữ COVER to (Text không tự xuống dòng)."""
    lines, cur = [], ""
    for w in text.split():
...
class BrandScene(Scene):
    ...
    # ---------- COVER / THUMBNAIL SEO (khung "nướng" ở ĐẦU video) ----------
    def _cover_hook(self):
        """Chữ hook cho cover: ưu tiên self.COVER (chỉnh tay/tập), else rút câu hỏi tò mò từ lời thoại."""
        c = getattr(self, "COVER", None)
        if isinstance(c, dict):
            c = c.get("hook", "")
        if isinstance(c, str) and c.strip():
            return c.strip()
        segs = list(self.SEGMENTS.values())[:2]                 # 2 đoạn đầu = hook
        text = " ".join(str(v).strip() for v in segs)
        sentences = [s.strip() for s in re.findall(r"[^.?!…]+[.?!…]?", text) if s.strip()]
        for s in sentences:                                     # ưu tiên câu hỏi gợi tò mò
            if s.endswith("?"):
                return s
        if sentences:
            return sentences[0]
        return self.TOPIC or CHANNEL_NAME

    def play_cover(self):
        """Render khung COVER chuẩn SEO TikTok (câu hỏi tò mò) và giữ TĨNH COVER_HOLD giây ở đầu video.
        TikTok lấy khung tại video_cover_timestamp_ms (config đăng) làm thumbnail hiển thị."""
        if not COVER_ON:
            return
        self.camera.background_color = BG
        glow = Ellipse(width=5.2, height=6.4, color=ACCENT).set_fill(ACCENT, 0.05) \
            .set_stroke(width=0).move_to([0, 0.6, 0]).set_z_index(-5)

        # kicker: chủ đề (tag nhận diện) ở trên
        dot = Dot(color=ACCENT, radius=0.07)
        topic = Text(self.TOPIC or CHANNEL_NAME, font=FONT, weight=BOLD, color=ACCENT).scale(0.5)
        kicker = fit_w(VGroup(dot, topic).arrange(RIGHT, buff=0.14), 3.4).move_to([0, 2.7, 0])

        # hook: câu hỏi tò mò — chữ LỚN, phóng cho vừa safe zone
        hook = Text(wrap_lines(self._cover_hook()), font=FONT, weight=BOLD, color=WHITE, line_spacing=1.05)
        hook.scale(min(COVER_MAXW / hook.width, COVER_MAXH / hook.height)).move_to([0, 0.55, 0])
        ul = Line(hook.get_left(), hook.get_right()).set_stroke(ACCENT, 5).next_to(hook, DOWN, buff=0.25)
        if ul.width > COVER_MAXW:
            ul.set_width(COVER_MAXW)

        # footer thương hiệu: mascot nhỏ + tên kênh
        mascot = mascot_parrot(0.28, smart=True)
        brand = Text(CHANNEL_NAME, font=FONT, weight=BOLD, color=WHITE).scale(0.42)
        footer = fit_w(VGroup(mascot, brand).arrange(RIGHT, buff=0.22), 3.4).move_to([0, -2.85, 0])

        cover = VGroup(glow, kicker, hook, ul, footer)
        self.add(cover)
        self.wait(COVER_HOLD)                                   # giữ tĩnh -> mọi khung giống nhau
        self.play(FadeOut(cover), run_time=0.25)

    # ---------- INTRO (dọc, ~3.5s) ----------
    def play_intro(self):
        self.play_cover()                                       # khung cover SEO trước intro
        self.camera.background_color = BG

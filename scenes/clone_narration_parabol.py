"""
Sinh giọng clone cho video "Parabol tia nước" (VoxCPM2, CPU).
Chạy:  ~/voxcpm-venv/bin/python scenes/clone_narration_parabol.py
Nguồn: narration_texts.py (SEGMENTS + tts_text — bản ĐỌC có phiên âm ký hiệu).
Mẫu: output/narration/voice-yeu_ref16k.wav · Xuất: output/narration_parabol/<id>.wav (48kHz).
"""
import os
import sys
import time
...
OUTDIR = os.path.join(BASE, "output", "narration_parabol")
...
def main():
    ...
    for name in SEGMENTS:
        text = tts_text(name)   # bản ĐỌC (ưu tiên phiên âm ký hiệu toán)
        t = time.time()
